...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Error: Resource group '$ResourceGroup' does not exist in subscription $SubscriptionId" -ForegroundColor Red
        exit 1
    }
    Write-Host "Resource group $ResourceGroup exists, proceeding with cleanup..." -ForegroundColor Green
    ...
    Write-Host "Error occurred during cleanup process: $_" -ForegroundColor Red
    exit 1
} finally {
    # Always clean up temporary files, even if the script fails
